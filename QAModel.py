...
class QAModel:
    ...
    def get_model(self):
        ...
        q = LSTM(self.question_len, dropout=0.05, return_sequences=True)(question_input)
        ...
        question_represent = Lambda(lambda x: K.sum(x, axis=1))(alpha_multiply)
        # evidence layer
    ...
    def testing_model(self):
        # question layer
        question_input = Input(shape=(self.question_len, self.word_dim), name='q_input')
        q = LSTM(64, dropout=0.05, return_sequences=True)(question_input)

        alpha_tanh = Dense(64, activation='tanh')(q)

        alpha = Dense(1, activation='softmax', name='alpha')(alpha_tanh)
        alpha_Multiply = Multiply()([q, alpha])
        question_represent = Lambda(lambda x: K.sum(x, axis=1))(alpha_Multiply)
        evidence_input = Input(shape=(self.evidence_len, self.word_dim), name='e_input', dtype=np.float32)
        question_represent_repeat = RepeatVector(self.evidence_len)(question_represent)
        ...
        lstm_2 = LSTM(64, return_sequences=True)(lstm_1)
        lstm_2_reverse = Lambda(lambda x: K.reverse(x, 0), name='lstm_reverse')(lstm_2)
        lstm_3 = LSTM(64, return_sequences=True)(concatenate([lstm_1, lstm_2_reverse]))
        # CRF layer
        tp_layer = TimeDistributed(Dense(4))(lstm_3)
        crf_layer = crf.ChainCRF(name='crf_layer')
        crf_o = crf_layer(tp_layer)
    ...
    def get_input_q_and_e(self, question, evidence):
...
if __name__ == '__main__':
    ...
    #
    # qamodel.fit([train_qs, train_es], [train_labels], epochs=10, batch_size=120)
